...
docker service create --publish 80:80 --network frontend --replicas 2 --name vote dockersamples/examplevotingapp_vote:before
...
docker service create --network frontend --replicas 2 --name redis redis:3.2
...
docker service create --network backend --mount type=volume,source=db-data,target=/var/lib/postgresql/data  --name db postgres:9.4
...
docker service create --publish 5001:80 --network backend --name result --constraint=node.role==manager \dockersamples/examplevotingapp_result:before